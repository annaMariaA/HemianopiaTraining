...
rtdat = rtdat[rtdat$hemiType=="Unmodified",]

accdat2 = aggregate(data=rtdat, acc ~ trial+ session+var +subj, FUN="mean")
errorbar <- summarySEwithin(accdat2, measurevar="acc", withinvars=c("session", "var", "subj"), idvar="trial")
pAcc2 = ggplot(errorbar, aes(x=session, y=100*acc, fill=var)) + geom_bar(stat="identity", position="dodge") + theme_minimal()
pAcc2 = pAcc2 + scale_y_continuous(name="Accuracy(%)") + scale_x_discrete(name="Target Position")+scale_fill_manual(name="Search Difficulty", values=cbPalette) + facet_wrap(~subj)
pAcc2 = pAcc2 + geom_errorbar(position=position_dodge(.9), within=.25, aes(ymin=(acc-ci)*100,ymax=(acc+ci)*100),width=.5)

ggsave("plots/accuracyUnmod.jpg",dpi=600, width=6, height=3)
