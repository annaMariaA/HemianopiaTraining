...
fixdat = fixdat[which(fixdat$trialType=="left" |fixdat$trialType=="right"),]
propSaccBlind = aggregate(data=fixdat, prop ~ subj+difficulty+session, FUN="mean")
p1 = ggplot(data=propSaccBlind, aes(x=session, y=prop, group=difficulty, colour=difficulty,group=supp))+geom_smooth(method=lm, size=1 )+geom_point(position=pd,size=2)
p1 = p1 + scale_y_continuous(limits=c(0.1, 1),name="Proportion first saccades into blind") + scale_x_discrete(name="Session")+ scale_color_manual(name="Difficulty",values=cbPalette)#+ facet_wrap(~subj)
p1= p1+ theme_minimal()
ggsave("plots/proportionAllSaccadesIntoBlindAllPP.jpg", width=10, height=3)
